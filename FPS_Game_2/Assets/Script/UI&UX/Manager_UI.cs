...
public class Manager_UI : MonoBehaviour
{
    ...
    public void TargetItem(RaycastHit target)
    {
        ...
        //Target not Found
        if (!target.transform.GetComponent<ItemPickUp>())
        {
            pointer.transform.gameObject.SetActive(false);
            return;
        }
    ...
    /// </summary>
    public void PlayerInfoSetWeapon(GunClass weapon)
    {
        Data_UI pointer = referenceManager[2];
        //Checks to make sure reference to PlayerInfo is not null.
        if (pointer == null)
        ...
        GunClass weaponTemp = weapon.transform.GetComponent<GunClass>();
        //Setting Weapon Icon
        ...
        pointer.referenceData[4].transform.GetComponent<TMP_Text>().text =
            weapon.currentAmmoCount + " | " + weapon.transform.GetComponentInParent<InventoryClass>().ammo;
    }
}
